...
This file will contain unit tests using pytest to test if your `Stack` and `solveMaze` functionality is correct. Think of various scenarios of utilizing a `Stack` correctly and of various mazes (with or without solutions, different sizes, and navigating to the Goal from various directions), and check to see if the traversal is correct according to these instructions. Write your tests first in order to check the correctness of your function. Again, Gradescope requires `testFile.py` to be submitted before running any autograded tests. Remember that testing can help you debug your algorithm and ensure your functionality works as expected!
